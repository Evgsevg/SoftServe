...
# GENERATE PASSOWRDS
# sudo yum -qy install openssl # openssl used for generating a truly random password
...
update() {
	...
	sudo yum -y -qq update 
}

installApache() {
	# Apache
	echo -e "\n ${Cyan} Installing Apache.. ${Color_Off}"
	sudo yum  -qy install apache2 apache2-doc libexpat1 ssl-cert
...
installLetsEncryptCertbot() {
  ...
  sudo yum update # update repo sources
  sudo yum install -y software-properties-common # required in order to add a repo
  sudo add-yum-repository ppa:certbot/certbot -y # add Certbot repo
  sudo yum update # update repo sources
  sudo yum install -y python-certbot-apache # install Certbot
}
...
installPHP() {
	...
	# yum install php5 libapache2-mod-php5 php5-cli php5-common php5-curl php5-dev php5-gd php5-intl php5-mcrypt php5-mysql php5-recode php5-xml php5-pspell php5-ps php5-imagick php-pear php-gettext -y
	...
	# sudo yum update
	# yum install php5.6 libapache2-mod-php5.6 php5.6-cli php5.6-common php-curl php5.6-curl php5.6-dev php5.6-gd php5.6-intl php5.6-mcrypt php5.6-mbstring php5.6-mysql php5.6-recode php5.6-xml php5.6-pspell php5.6-ps php5.6-imagick php-pear php-gettext -y

	# PHP7 (latest)
	sudo yum -qy install php php-common libapache2-mod-php php-curl php-dev php-gd php-gettext php-imagick php-intl php-mbstring php-mysql php-pear php-pspell php-recode php-xml php-zip
}
...
installMySQL() {
	...
	DEBIAN_FRONTEND=noninteractive sudo yum -qy install mysql-server mysql-client
}
...
installPHPMyAdmin() {
	...
	DEBIAN_FRONTEND=noninteractive sudo yum -qy install phpmyadmin
}
